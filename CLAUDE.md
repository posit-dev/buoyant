...
`buoyant` is an R package for deploying `_server.yml` compliant web
applications to DigitalOcean. It supports any R web framework that
implements the `_server.yml` standard (plumber2, fiery, etc.).
...
- `devtools::test(filter = "then")` - Run tests matching pattern (e.g.,
  all tests in test-then.R)
- `testthat::test_file("tests/testthat/test-*.R")` - Run a single test
  file
...
The package uses RStudio’s shiny-workflows for CI/CD: - Automated R CMD
check on push/PR - Website deployment via pkgdown - Code formatting
checks
...
- [`do_configure_https()`](https://posit-dev.github.io/buoyant/reference/do_configure_https.md) -
  Add HTTPS/SSL support via Let’s Encrypt
...
- [`read_server_yml()`](https://posit-dev.github.io/buoyant/reference/read_server_yml.md) -
  Read and parse a \_server.yml configuration
...
3.  **Automatic Setup**: Creates systemd services and nginx
    configuration
...
1.  **Validate** the \_server.yml file with
    [`validate_server_yml()`](https://posit-dev.github.io/buoyant/reference/validate_server_yml.md)
2.  **Provision** a DigitalOcean server with
    [`do_provision()`](https://posit-dev.github.io/buoyant/reference/do_provision.md)
3.  **Deploy** the application with
    [`do_deploy_server()`](https://posit-dev.github.io/buoyant/reference/do_deploy_server.md)
4.  **Access** via the URL from
    [`do_ip()`](https://posit-dev.github.io/buoyant/reference/do_ip.md)
...
The package includes unit tests for: - Validation functions (detecting
errors, parsing configs)
...
Potential additions: - Support for additional deployment targets (AWS,
Azure, Heroku) - Docker container generation - Load balancing
configuration - Database setup helpers - Monitoring and logging
integration - CI/CD pipeline templates
...
Inspired by [plumberDeploy](https://github.com/meztez/plumberDeploy) by
Bruno Tremblay and Jeff Allen.

The \_server.yml standard was developed as part of the
[plumber2](https://plumber2.posit.co) project.
