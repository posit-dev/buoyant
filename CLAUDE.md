...
│   ├── templates.R         # Template creation functions
...
│       ├── plumber2/       # plumber2 template
│       │   ├── _server.yml
│       │   └── api.R
│       ├── fiery/          # fiery template
│       │   ├── _server.yml
│       │   └── app.R
...
6. **Templates**: Quick start with pre-built templates
7. **Validation**: Validate configurations before deployment
...
1. **Create** an application locally using `create_template()`
2. **Validate** the _server.yml file with `validate_server_yml()`
3. **Provision** a DigitalOcean server with `do_provision()`
4. **Deploy** the application with `do_deploy_server()`
5. **Access** via the URL from `do_ip()`
6. **Monitor** using SSH and systemd commands
7. **Update** by redeploying with `overwrite = TRUE`
8. **Remove** with `do_remove_server()` when done
...
| Templates | Yes (multiple engines) | Yes (plumber) |

## Testing

The package includes unit tests for:
- Validation functions (detecting errors, parsing configs)
- Template functions (creating, listing, overwriting)
